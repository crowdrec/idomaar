...
def evalRecall(x,NList=[1,2,5,10]):
    GTList = set([int(k['object']['id']) for k in x['GT']['expected']['evidences']])
    res = {}
    for N in NList:
        recList = set([int(k['id']) for k in x['rec'] if int(k['rank']) <= N ])
        if N == -1:
            recList = set([int(k['id']) for k in x['rec']])
            N = 'hits'
        res[N] = len(GTList & recList)
    res['GtLength'] = len(GTList)
    res['RecLength'] = len(set([int(k['id']) for k in x['rec']]))
    return res


def testAndTry(x):
    try: 
        return True,json.loads(x[7])['GT'],json.loads(x[7])['rec']
    except Exception:
